            ...
            <div className="max-lg:w-full flex flex-wrap flex-col gap-3 w-36">
                ...
                {!user?.portfolio_links ? null : (
                    user.portfolio_links.map((link) => {
                        return (
                            <Link key={link} to={link} rel="noopener" target="_blank" className="inline-block text-sm lg:text-gray-500 pr-3 max-lg:mt-4 max-lg:font-semibold hover:text-primary">
                                {link}
                            </Link> 
                        )
                    })
