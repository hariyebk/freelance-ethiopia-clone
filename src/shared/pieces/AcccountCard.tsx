...
export default function AcccountCard({name, role}: AccountCardProps) {
    ...
    return (
        ...
        <div className="max-lg:w-[400px] w-full h-[100px] max-lg:mt-6 mt-3 bg-white shadow-md rounded-md flex items-center justify-between gap-5 px-4 py-7">
        ...
        <div className="w-full max-lg:mt-7 mt-5 ml-3 flex flex-col items-start">
            ...
            <button className= {`${!selected ? "bg-stone-800 opacity-50" : "bg-gradient-to-r from-primary to-secondary"} mt-14 ml-20 max-lg:text-base text-sm text-slate-100 cursor-pointer rounded-full px-7 py-2`} disabled = {!selected}> Choose Account Type </button>
        </div>
        </div>
    )
}
