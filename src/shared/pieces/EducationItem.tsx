...
export default function EducationItem({isEditing, institute, enrolled_in, fieldOfStudy, startDate, finishedDate}: EducationProps) {
    return (
        <div className={`${isEditing && "lg:pl-12"} mt-5 w-full flex flex-col items-start`}>
            ...
            <p className="mt-3 text-sm text-black font-palanquin font-semibold"> From: <span className="capitalize"> {formatDateString(startDate)} </span> <span className="mx-1"> - </span> To: <span className="capitalize"> {formatDateString(finishedDate)} </span> </p>
        </div>
    )
}
